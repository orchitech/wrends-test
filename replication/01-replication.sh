...
exec_ds 2 ./bin/ldapsearch --port 1389 --hostname localhost --bindDN "cn=Directory Manager"  --bindPassword password \
  --baseDN dc=example,dc=com "(uid=user.0)" uid | expect_no_result

log_message "Setting up replication"
exec_ds 1 ./bin/dsreplication enable --adminUID admin --adminPassword password --trustAll --no-prompt --baseDN dc=example,dc=com \
...
exec_ds 1 ./bin/dsreplication initialize-all --adminUID admin --adminPassword password --trustAll --no-prompt \
...
exec_ds 2 ./bin/ldapdelete --port 1389 --hostname localhost --bindDN "cn=Directory Manager"  --bindPassword password \
